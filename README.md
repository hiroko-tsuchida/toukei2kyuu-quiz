...
- **図解つき解説** … 正規分布の面積・棄却域・信頼区間・箱ひげ図など、言葉だけでは伝わりにくい34問に図を表示
- **スコア集計** … 最後に正解率と判定を表示

全170問（🟢易50＝10LV・🟡標準50＝10LV・🔴難30＝6LV・🟣実践30＝6LV・🟣CBT式実践10＝2LV、計34LV。CBT式実践は本番CBTと同じ5択・数表の値つきの形式）／10分野（1変数データ／2変数データ／時系列データ／データ収集／確率／確率分布／標本分布／推定／仮説検定／回帰・分散分析）。
